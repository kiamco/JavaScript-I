...
const getLastObject = () => {
    let lastObj = inventory[inventory.length - 1];
    return lastObj;
}

let lastCar = getLastObject();
...
console.log('sorted model list:')
console.log(sortByModel(inventory));
console.log('-------------------------------------------------------------');
...
let oldCars = [];
allCarYears = getCarYear();
for (i = 0; i < allCarYears.length; i++) {
    if (allCarYears[i] < 2000) {
        oldCars.push(allCarYears[i]);
    }
}
